...
def main():
    ...
    if not processed_path.exists():
        print(f"✗ Error: Processed data directory not found at {processed_path}")
        return
    ...
    if not jsonl_files:
        print(f"⚠️  No processed data files found in {processed_path}")
        return
        ...
        if chunks:
            ...
            for namespace, ns_chunks in namespace_map.items():
                try:
                    ...
                    print(f"     ✔ Added to namespace '{namespace}'")
                except Exception as e:
                    print(f"     ✗ Failed to add to namespace '{namespace}': {e}")
